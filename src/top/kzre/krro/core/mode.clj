(ns top.kzre.krro.core.mode
  "模式系统，参照 Emacs 的 major/minor-mode。
   模式是数据，激活时接管键图、布局、局部变量与 Hook。
   Hook 直接基于 mode-id 生成，例如 :krro.painting/painting 生成 :painting-mode-enter-hook。
   错误通过消息系统输出。"
  (:require
    [top.kzre.krro.core.command :as cmd]
    [top.kzre.krro.core.custom :as custom]
    [top.kzre.krro.core.frame :as frame]
    [top.kzre.krro.core.hook :as hook]
    [top.kzre.krro.core.keymap :as km]
    [top.kzre.krro.core.message :as msg]
    [top.kzre.krro.core.ui.protocol :as ui]
    [top.kzre.krro.core.util.naming :as naming]
    [top.kzre.krro.core.window :as win]))


;; ── Hook 关键字生成 ──────────────────────────────
(defn- enter-hook-key [mode-id]
  (naming/naming-keyword-subfix mode-id "-mode-enter-hook"))

(defn- exit-hook-key [mode-id]
  (naming/naming-keyword-subfix mode-id "-mode-exit-hook"))

;; ── 模式注册表 ──────────────────────────────────
(defonce ^:private mode-registry (atom {}))
;; TODO name 换成可选参数
(defn make-major-mode
  [id  & {:keys [name parent keymap layout variables after-hook]
              :or {parent :krro.core/fundamental}}]
  (cond-> {:id id, :name (or name (str id)), :parent parent}
          keymap  (assoc :keymap (km/keymap* keymap))
          layout  (assoc :layout layout)
          variables (assoc :variables variables)
          after-hook (assoc :after-hook after-hook)))

(defn make-minor-mode
  [id  & {:keys [name keymap variables after-hook]}]
  (cond-> {:id id, :name (or name (str id)), :minor? true}
          keymap     (assoc :keymap (km/keymap* keymap))
          variables  (assoc :variables variables)
          after-hook (assoc :after-hook after-hook)))

(defn register-mode! [spec] (swap! mode-registry assoc (:id spec) spec))
(defn get-mode-spec [id] (get @mode-registry id))


(defn keymaps
  "根据 Frame 的当前模式状态动态构建有效的键图列表。
   优先级从高到低：
   1. 所有激活的副模式键图（顺序任意）
   2. 主模式及其父链键图（子优先，即当前主模式最先）
   3. 全局键图
   返回键图向量列表。"
  ([] (keymaps (win/active-frame)))
  ([f]
   (let [major-id (frame/major-mode f)
         minors   (frame/minor-modes f)
         ;; 收集副模式键图
         minor-kms (keep #(when-let [spec (get-mode-spec %)]
                            (:keymap spec))
                         minors)
         ;; 沿父链向上收集主模式键图，再反转使得子模式在前
         major-chain (take-while some? (iterate (fn [id]
                                                  (when-let [spec (get-mode-spec id)]
                                                    (:parent spec)))
                                                major-id))
         major-kms   (reverse (keep #(when-let [spec (get-mode-spec %)]
                                       (:keymap spec))
                                    (cons major-id major-chain)))]
     (vec (concat minor-kms major-kms [@km/global-keymap])))))




(defn- resolve-variables
  "沿主模式父链合并变量，子模式覆盖父模式。
 副模式没有父链，返回自身变量。"
  [mode-id]
  (loop [id mode-id, vars {}]
    (if-let [spec (get-mode-spec id)]
      (let [parent (when-not (:minor? spec) (:parent spec))]
        (recur parent (merge (:variables spec) vars)))
      vars)))

(defn- parent-chain [mode-id]
  (take-while some?
              (iterate (fn [id]
                         (when-let [spec (get-mode-spec id)]
                           (:parent spec)))
                       (when-let [spec (get-mode-spec mode-id)]
                         (:parent spec)))))


;; ═══════════════════════════════════════════════════════
;; 模式激活与切换
;; ═══════════════════════════════════════════════════════



(defn- enter-major-mode! [mode-id f]
  (let [spec (get-mode-spec mode-id)
        all-vars (resolve-variables mode-id)]
    (doseq [[id value] all-vars]
      (custom/set-custom-local! id value f))
    (if-let [layout (:layout spec)]
      (ui/render-frame! layout f)
      (msg/warn (str "major-mode without layout "  mode-id)))
    (doseq [mid (reverse (cons mode-id (parent-chain mode-id)))]
      (hook/run-hook! (enter-hook-key mid) f)) ))

(defn- exit-major-mode! [mode-id f]
  (let [all-vars (resolve-variables mode-id)]
    (doseq [id (keys all-vars)]
      (custom/kill-local-custom! id f))
    (doseq [mid (cons mode-id (parent-chain mode-id))]
      (hook/run-hook! (exit-hook-key mid) f))))

;; 工具：统一变量格式（副模式没有父链，但仍可处理两种格式）
(defn- normalize-variables [vars]
  (into {} (for [[id spec] vars]
             (if (map? spec)
               [id (:default spec)]    ;; 兼容 {:id {:default v}}
               [id spec]))))           ;; 新格式 {:id v}

;; ── 副模式进入 ──────────────────────
(defn- enter-minor-mode! [mode-id f]
  (let [all-vars (resolve-variables mode-id)]
    (doseq [[id value] all-vars]
      (custom/set-custom-local! id value f)))
  (hook/run-hook! (enter-hook-key mode-id) f))

(defn- exit-minor-mode! [mode-id f]
  (doseq [id (keys (resolve-variables mode-id))]
    (custom/kill-local-custom! id f))
  (hook/run-hook! (exit-hook-key mode-id) f))


(defn activate-major-mode!
  ([mode-id] (activate-major-mode! mode-id (win/active-frame)))
  ([mode-id f]
   (if (get-mode-spec mode-id)
     (if-let [old-major-id (frame/major-mode f)]
       (when (not= old-major-id mode-id)
         (exit-major-mode! old-major-id f)
         (frame/set-major-mode! f mode-id)
         (enter-major-mode! mode-id f))
       (msg/error "frame without major mode"))
     (msg/error (str "Mode not registered: " mode-id)))))


(defn activate-minor-mode!
  ([mode-id] (activate-minor-mode! mode-id (win/active-frame)))
  ([mode-id f]
   (if (get-mode-spec mode-id)
     (let [active-minors (frame/minor-modes f)]
       (when-not (contains? active-minors mode-id)
         (enter-minor-mode! mode-id f)
         (frame/add-minor-mode! f mode-id)
         mode-id))
     (msg/error (str "Minor mode not registered: " mode-id)))))

(defn fundamental-activate!
  ([] (fundamental-activate! (win/active-frame)))
  ([f] (activate-major-mode! :krro.core/fundamental f)))

(defn deactivate-major-mode!
  ([mode-id] (deactivate-major-mode! mode-id (win/active-frame)))
  ([mode-id f]
   (when (not= :krro.core/fundamental mode-id)
     (exit-major-mode! mode-id f)
     (fundamental-activate! f))))

(defn deactivate-minor-mode!
  ([mode-id] (deactivate-minor-mode! mode-id (win/active-frame)))
  ([mode-id f]
   (if (get-mode-spec mode-id)
     (let [active-minors (frame/minor-modes f)]
       (when (contains? active-minors mode-id)
         (exit-minor-mode! mode-id f)
         (frame/remove-minor-mode! f mode-id)
         mode-id))
     (msg/error (str "Minor mode not registered: " mode-id)))))

(defn toggle-minor-mode!
  ([mode-id] (toggle-minor-mode! mode-id (win/active-frame)))
  ([mode-id f]
   (if (contains? (frame/minor-modes f) mode-id)
     (deactivate-minor-mode! mode-id f)
     (activate-minor-mode! mode-id f))))



(defn define-major-mode
  "注册一个 major mode 并生成激活/停用命令。
   参数：
     - mode-id: mode 的唯一标识符（关键字）
     - opts:    额外的键值对选项，会原样传递给 mode/make-major-mode"
  [mode-id & {:keys [name] :as opts}]
  (let [activate-cmd   (naming/naming-keyword-around mode-id "activate-" "-mode!")
        deactivate-cmd (naming/naming-keyword-around mode-id "deactivate-" "deactivate-mode!")]
    (register-mode! (apply make-major-mode mode-id (flatten (seq opts))))
    (cmd/reg-command activate-cmd
                           (fn [project] (activate-major-mode! mode-id) project)
                           :description (str "Activate " name " mode"))
    (cmd/reg-command deactivate-cmd
                           (fn [project] (deactivate-major-mode! mode-id) project)
                           :description (str "Deactivate " name " mode"))))


(defn define-minor-mode
  "定义副模式并注册 activate / deactivate / toggle 命令。
   参数：
     - mode-id: mode 的唯一标识符（关键字）
     - name:    mode 的显示名称（字符串）
     - opts:    额外的键值对选项，会原样传递给 mode/make-minor-mode"
  [mode-id  & {:keys [name] :as opts}]
  (let [activate-cmd   (naming/naming-keyword-around mode-id "activate-" "-mode!")
        deactivate-cmd (naming/naming-keyword-around mode-id "deactivate-" "-mode!")
        toggle-cmd     (naming/naming-keyword-around mode-id "toggle-" "-mode!")]
    (register-mode! (apply make-minor-mode mode-id (flatten (seq opts))))
    (cmd/reg-command activate-cmd
                           (fn [project] (activate-minor-mode! mode-id) project)
                           :description (str "Activate " name " minor mode"))
    (cmd/reg-command deactivate-cmd
                           (fn [project] (deactivate-minor-mode! mode-id) project)
                           :description (str "Deactivate " name " minor mode"))
    (cmd/reg-command toggle-cmd
                           (fn [project] (toggle-minor-mode! mode-id) project)
                           :description (str "Toggle " name " minor mode"))))